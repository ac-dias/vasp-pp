...
!gfortran vasp_bands_kpath_fake_procar.f90 -o vbands_k_f_p.x
! vbands_k_f_p.x 6 20 lorbit >bands.dat
!6 numero linhas kpath com os pontos do caminho
!20 numero de pontos com peso diferente de 0
...
program main
	...
	OPEN(UNIT=101, FILE= "KPOINTS_FAKE",STATUS='old', IOSTAT=erro)
	...
	OPEN(UNIT=103, FILE= "PROCAR",STATUS='old', IOSTAT=erro)
    	if (erro/=0) stop "Erro na abertura do arquivo de entrada 4"

      CALL getarg(1, arg)
		read(arg,*,iostat=ios) nptpath

      CALL getarg(2, arg)
		read(arg,*,iostat=ios) ifake

      CALL getarg(3, arg)
		read(arg,*,iostat=ios) lorbit
	...
	allocate(energies(nkpoints,nbands,3))
	...
	allocate(pflag2(nread-1))

	read(103,*) cflag
	read(103,*) cflag,cflag,cflag,pflag,cflag,cflag,cflag,pflag,cflag,cflag,cflag,nions
	...
	read(103,*) cflag!,pflag,cflag,pflag,pflag,pflag,cflag,cflag,pflag
	 ...
	 do j=1,nbands
	  ...
	  read(103,*) cflag!,pflag,cflag,cflag,pflag,cflag,cflag,pflag
	  read(103,*) 
	...
	do j=1,nbands
		...
		do i=1,(nptpath/2)*nptrechos
			...
			write(*,*) kpathp(i),energies(i+(ifake),j,2),energies(i+(ifake),j,3)
